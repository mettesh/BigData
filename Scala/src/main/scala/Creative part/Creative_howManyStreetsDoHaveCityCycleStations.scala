...
object Creative_howManyStreetsDoHaveCityCycleStations {

  // I hvor mange av gatene i osm-fila har bysykkel-stasjoner?

  def main(args: Array[String]) {

    val spark: SparkSession = initializeSpark(args)
    val wayData : DataFrame = searchForNodeAndTag(spark)
    val cityCyclingData : DataFrame = searchForNodeAndTagCityCycling(spark)

    import spark.implicits._

    val names = cityCyclingData.select("station_id", "name")
  ...
  private def initializeSpark(args: Array[String]) = {
    val conf = new SparkConf().setMaster(args(0)).setAppName("Creative task 1")
  ...
  def searchForNodeAndTagCityCycling(spark: SparkSession): DataFrame = {
    ...
    val data = spark.read.json("input/bysykkel.json")
    data
